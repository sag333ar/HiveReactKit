...
const VideoFeed = ({
  ...
  const convertToVideoItem = useCallback(
    async (video: any): Promise<VideoFeedItem | null> => {
      ...
      if (video.author?.username) {
        ...
        const thumbnail =
          formatThumbnailUrl(spkvideoData?.thumbnail_url) ||
          formatThumbnailUrl(video.json_metadata?.raw?.video?.info?.thumbnail);
        const duration =
          spkvideoData?.duration ||
          video.json_metadata?.raw?.video?.info?.duration ||
          0;
        ...
        return {
          ...
          duration: duration,
          thumbnail: thumbnail,
          numOfUpvotes: video.stats?.num_votes || video.numOfUpvotes,
          numOfComments: video.stats?.num_comments || video.numOfComments,
          hiveValue: video.stats?.total_hive_reward || video.hiveValue,
        };
      }
      ...
      try {
        ...
        switch (feedType) {
          case ApiVideoFeedType.SEARCH:
            if (tag) {
              // tag will be used as search term for search feed
  ...
  useEffect(() => {
    const loadInitial = async () => {
      setLoading(true);
      setVideos([]); // Clear previous tab's data immediately
      setSkip(0);

      const cached = feedCache.get(cacheKey);
      if (cached?.length) {
        // Immediately display cached data for the new tab
        setVideos(cached);
        setLoading(false); // Stop loading indicator since we have cached data
      }
